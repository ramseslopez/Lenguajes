module BAE.Sintax where
  ...
  subst (Fix x e) (c, d)
                          | elem x (frVars d) = Fix (incrVar x) (subst e (c, d))
                          | otherwise = Fix x (subst e (c, d))
  ...
  alphaExpr (Add e1 e2) = Add (alphaExpr e1) (alphaExpr e<2)
  ...
  alphaExpr (Fix x e) = Fix (incrVar x) (alphaExpr e)
