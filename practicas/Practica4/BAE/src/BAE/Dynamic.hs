    ...
    eval1 (Add e1 e2) = (Add (eval1 e1) (eval1 e2))
    eval1 (Mul (I n) (I m)) = (I (n * m))
    eval1 (Mul e1 e2) = (Mul (eval1 e1) (eval1 e2))
    ...
    eval1 (And e1 e2) = (And (eval1 e1) (eval1 e2))
    eval1 (Or (B t) (B f)) = if t == False && f == False then (B False) else (B True)
    eval1 (Or e1 e2) = (Or (eval1 e1) (eval1 e2))
    eval1 (Lt (I n)(I m)) = if n < m then (I n) else (I m)
    eval1 (Lt e1 e2) = (Lt (eval1 e1) (eval1 e2))
    eval1 (Gt (I n)(I m)) = if n > m then (I n) else (I m)
    eval1 (Gt e1 e2) = (Gt (eval1 e1) (eval1 e2))
    eval1 (Eq (I n) (I m)) = if n == m then B True else B False
    eval1 (Eq e1 e2) = (Eq (eval1 e1) (eval1 e2))
    eval1 (If (B t) a b) = if t then a else b
    eval1 (If e1 e2 e3) = (If (eval1 e1) (eval1 e2) (eval1 e3))
    eval1 (Let x (I n) e2) = subst e2 (x, (I n))
    eval1 (Let x (B b) e2) = subst e2 (x, (B b))
    eval1 (Let x e1 e2) = Let x (eval1 e1) (eval1 e2)
    ...
    eval :: Expr -> Type -> Expr
    eval (I n) Integer = (I n)
    eval (B b) Boolean = (B b)
    ...
    eval (Eq e1 e2) t = if t == Boolean then evals(Eq e1 e2) else error "El tipo no corresponde"(m ++ [(n, e2)], Void)
    eval _ _ = error "El tipo no corresponde"(m ++ [(n, e2)], Void)
